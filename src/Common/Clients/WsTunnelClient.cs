...
public class WsTunnelClient : ITunnelClient
{
  record ClientInfo(WebsocketClient WsClient, ILifetime Lifetime)
  {
    ...
    ILog _logger)
  {
    ...
    var localServiceSocket = _lifetime.ToDisposeOnEnded(new Socket(p_localEndpoint.Address.AddressFamily, SocketType.Dgram, ProtocolType.Udp));
    try
    {
      localServiceSocket.Bind(p_localEndpoint);
    }
    ...
    IReadOnlyLifetime _lifetime)
  {
    ...
    var cryptor = EncryptionToolkit.GetCrypto(p_options.EncryptionAlgorithm, _lifetime, p_options.PassKey);
    ...
    while (!_lifetime.IsCancellationRequested)
    {
      try
      {
        ...
        if (receivedBytes > 0)
        {
          ...
          client.WsClient.SendInstant(dataToSend.ToArray());
  ...
  private async Task CreateRemoteServerRoutineAsync(
    ...
    _wsClient.MessageReceived
      .Subscribe(_msg =>
      {
        try
        {
          ...
          //p_logger.Info($"Got packet {bytes.Length} bytes");
    ...
    EndPoint _localServiceEndpoint)
  {
    ...
    if (lifetime != null)
    {
      var wsClient = new WebsocketClient(new Uri($"{p_options.RemoteEndpoint}?key={p_options.PassKey}"))
      {
      ...
      return new(wsClient, lifetime) { Timestamp = Environment.TickCount64 };
    }
